...
export default function TableEditor({ initialData, filename, textId = '' }: Props) {
  ...
  const handleBlockDrop = (fromRow: number, fromField: string, toRow: number, toField: string) => {
    setData(prev => {
      const d = [...prev]
      // MUST clone the row objects so React detects the state change
      d[fromRow] = { ...d[fromRow] }
      d[toRow] = { ...d[toRow] }
  ...
  const saveSingleRow = async (idx: number) => {
    ...
    try {
      ...
      // Append auto-notes to existing notes (avoid duplicates)
...
function LangCell({ v1, proposed, rowIdx, lang, isMarker, isImproving, onV1Change, onProposedChange, onImprove, onWordClick, onBlockDrop }: {
  ...
  const handleBlockDrop = (e: React.DragEvent, toField: string) => {
    e.preventDefault()
  ...
  const acceptAnnotation = (idx: number) => {
    ...
    // Remove accepted annotation and adjust indices for remaining annotations
  ...
  const renderAnnotatedText = () => {
    ...
    sorted.forEach((ann, i) => {
      ...
      // Annotation: old (red strikethrough) + new (green bold)
      parts.push(
        <span key={`a${i}`} style={{ display: 'inline', position: 'relative' }}>
          ...
          <span style={{ 
            display: 'inline-flex', gap: 1, marginLeft: 2, verticalAlign: 'middle'
          }}>
          ...
          <span style={{ 
            fontSize: '0.5rem', color: '#8b5cf6', background: '#f5f3ff', 
            padding: '0 3px', borderRadius: 8, marginLeft: 2, fontWeight: 600,
    ...
    <td style={{ padding: '7px 8px', verticalAlign: 'top', borderRight: '1px solid #e9edf2' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        ...
        <div 
          ...
          onDragLeave={handleBlockDragLeave}
          onDrop={e => handleBlockDrop(e, 'proposed')}
          style={{ 
        ...
        >
          ...
          <textarea value={proposed || ''} onChange={e => onProposedChange(e.target.value)}
            onClick={e => onWordClick(e, rowIdx, lang)} placeholder="Tasdiqlangan matn..."
